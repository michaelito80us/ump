...
### 1.5 Primary Personas & Key Use‑Cases

| Persona       | Motivation                            | Core Jobs‑to‑be‑Done                                             |
| ------------- | ------------------------------------- | ---------------------------------------------------------------- |
| **Org Admin** | Publish tournaments fast & error‑free | Create new tournament, invite referees, monitor schedule changes |
| **Referee**   | File official scores from court‑side  | Start match timer, enter score, approve contested points         |
| **Coach**     | Keep team logistics tight             | Manage roster, submit line‑ups, receive schedule changes push    |
| **Player**    | Track own schedule & stats            | See next match, personal stats, receive notifications            |
| **Spectator** | Follow live action                    | Real‑time scores, bracket view                                   |

Personas are distilled from feedback interviews with 7 regional federations and should guide copy tone, navigation order and feature trade‑offs.

### 1.6 Success Metrics & KPIs

| Metric                         | Target              | Notes                               |
| ------------------------------ | ------------------- | ----------------------------------- |
| **Time‑to‑publish tournament** | ≤ 10 min P95        | Admin from “New” to link shareable  |
| **Live score latency**         | < 200 ms P95        | Edge to WebSocket client round‑trip |
| **Mobile Core Web Vitals**     | 90+ (LCP, FID, CLS) | Lighthouse on Moto G4 profile       |
| **Crash‑free sessions**        | ≥ 99.8 %            | Sentry daily window                 |

KPIs are wired to Grafana dashboards and dictate perf budgets and refactor priority.

### 1.7 Content Style Guide (Voice & Glossary)

> **Voice** – Friendly, concise, avoids sporting jargon unless unavoidable. 2nd‑person, imperative: “Tap **Start match**”.
>
> **Terminology**
>
> - **Game** – Smallest scoreable unit (e.g. volleyball rally),
> - **Match** – Contest between two teams,
> - **Stage** – Group, bracket, pool etc.

All micro‑copy must reference this glossary to stay translatable and prevent confusion across sports.

### 1.8 End‑to‑End Happy‑Path Flow

```
(Admin) Create Tournament → Add Venues → Publish Registration → Close Reg → Auto‑Seed Bracket → Referees Report Scores → Podium Share
```

Sketch the above in Figma wire‑flows before implementing screens; edge‑cases stand out quickly (e.g. venue clash).

### 1.9 Design Tokens & Visual Identity

| Token             | Value               | Rationale                                |
| ----------------- | ------------------- | ---------------------------------------- |
| `--color‑primary` | `#0066EE`           | Visible under sunlight, WCAG AA on white |
| `--radius‑md`     | `0.5rem`            | Matches iOS default card rounding        |
| `--font‑heading`  | `Inter, sans‑serif` | Open‑source, high legibility             |

Tokens are exported to `tailwind.config.ts` and consumed by shadcn/ui themes.

### 1.10 Security & Compliance Baseline

- **OWASP Top‑10 mitigations** – CSP, HSTS, rate‑limit, CSRF double‑submit.
- **Threat Model Diagram** – Data‑flow + trust boundaries (draw.io link).
- **GDPR** – Data Processing Agreement template, user data export endpoint.

These controls are minimum viable for public beta; enumerated in Section 12 tasks.

### 1.11 Architecture Decision Records (ADR)

For every significant trade‑off (e.g. "Why **Prisma** over **TypeORM**") record an ADR in `/docs/adr/`. Follow [MADR 2.1](https://adr.github.io/madr/) structure.
