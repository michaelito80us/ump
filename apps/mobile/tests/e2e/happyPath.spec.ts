...
describe('Tournament Happy Path E2E', () => {
    ...
    it('should allow organizer to create a new tournament', () => {
      ...
      cy.fixture('tournament').then((tournamentData) => {
        ...
        cy.typeWithoutKeyboard(
          ...
          tournamentData.tournament.maxTeams.toString()
        );
        cy.dataTestId('min-players').clear();
        cy.typeWithoutKeyboard(
          '[data-testid="min-players"]',
          tournamentData.tournament.minPlayersPerTeam.toString()
        );
        cy.dataTestId('max-players').clear();
        cy.typeWithoutKeyboard(
          '[data-testid="max-players"]',
          tournamentData.tournament.maxPlayersPerTeam.toString()
        );
        ...
        cy.get('body').then(($body) => {
          if (
            $body.text().includes('Tournament created successfully') ||
            $body.text().includes('Tournament published')
          ) {
  ...
  describe('Phase 3: Tournament Execution', () => {
    it('should display tournament schedule to spectators', () => {
      ...
      cy.fixture('matches').then((matchData) => {
        ...
        expect(matchData)
          ...
          .and.have.property('matches');
        expect(matchData.matches)
          .to.be.an('array')
          .and.have.length.greaterThan(0);

        // Verify matches are displayed with dynamic IDs
        cy.get('[data-testid^="match-card-"]').should(
        ...
        cy.get('[data-testid^="match-card-"]')
          ...
          .then(($card) => {
            ...
            if (testId) {
              ...
              cy.get(`[data-testid="match-status-${matchId}"]`).should(
                'be.visible'
              );
            }
          });
        ...
        cy.log(
          'Looking for live matches in:',
          JSON.stringify(matchData.matches)
        );
        const liveMatch = matchData.matches.find(
          (m: any) => m.status === 'LIVE'
        );
        ...
        if (liveMatch && liveMatch.teamA && liveMatch.teamA.name) {
          ...
          cy.log(
            'No valid live match found in fixture data or missing team data'
          );
        }
      });
    });
    ...
    it('should simulate match progression and update standings', () => {
      ...
      cy.fixture('matches').then((matchData) => {
        const pendingMatch = matchData.matches.find(
  ...
  describe('Phase 4: Tournament Completion', () => {
    it('should display final results and champion', () => {
      ...
      cy.fixture('matches').then((matchData) => {
        cy.fixture('tournament').then((tournamentData) => {
          const completedMatch = matchData.matches.find(
            (m: any) => m.status === 'COMPLETED'
          );
          const champion = completedMatch
            ? completedMatch.winner
            : tournamentData.teams[0].id;
          const championTeam = tournamentData.teams.find(
            (t: any) => t.id === champion
          );
    ...
    it('should provide tournament archive and statistics', () => {
      ...
      cy.visit(
        'http://localhost:3001/en/tournaments/' + tournamentSlug + '/archive'
      );
      ...
      cy.get('[data-testid^="live-match-card-"]')
        ...
        .then(($card) => {
          ...
          if (testId) {
            ...
            cy.get(`[data-testid="live-team-a-name-${matchId}"]`).should(
              'be.visible'
            );
            cy.get(`[data-testid="live-team-a-score-${matchId}"]`).should(
              'be.visible'
            );
            cy.get(`[data-testid="live-team-b-name-${matchId}"]`).should(
              'be.visible'
            );
            cy.get(`[data-testid="live-team-b-score-${matchId}"]`).should(
              'be.visible'
            );
  ...
  describe('Error Handling and Edge Cases', () => {
    it('should handle network connectivity issues gracefully', () => {
      ...
      cy.contains('You are offline. Some features may be limited.').should(
        'be.visible'
      );
